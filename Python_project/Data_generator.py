###This file allows to generate synthetic data, by exploiting the probabilistic model described in the report.
#The hyperparameters of the model were fitted thanks to statistics on the real data. See the Event_labeling.py function
#and the functions histH,histA,histB,histS,histP.
...
#This function returns labelled artificial data
    ...
    #The values of these parameters were obtained exlpoiting the distribution results of the Event_labeling.py file
    #See the functions histS,histP,histH,histA and histB.
    mu=0
    sigma0=5.866
    p=0.0009
    a=-14
    ...
    datM=dat.rolling(100).mean() #Applying moving average to smooth the data
    S1=np.std(dat['S'])
    S2=np.std(datM['S'])
    datM=datM*(S1/S2)

    #Applying the savgol_filter two times
    ...
    while j<N :
        ...
        a=-np.random.gamma(alphaA,1/betaA) #a,b are the parameters of the exponential in the fit
